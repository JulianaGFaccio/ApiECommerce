...
public class ItemCarrinhoController : ControllerBase
{
    ...
    public ItemCarrinhoController(AppDbContext dbContext)
    {
    ...
    public async Task<IActionResult> Get(int usuarioId)
    {
        ...
        var itemCarrinho = await (from s in dbContext.ItensCarrinhoCompra.Where(s => s.ClienteId == usuarioId)
    ...
    public async Task<IActionResult> Post([FromBody] ItemCarrinhoCompra itemCarrinhoCompra)
    {
        try
        {
            var carrinhoCompra = await dbContext.ItensCarrinhoCompra.FirstOrDefaultAsync(s => s.ProdutoId == itemCarrinhoCompra.ProdutoId && s.ClienteId == itemCarrinhoCompra.ClienteId);
            ...
            else
            {
                ...
                var carrinho = new ItemCarrinhoCompra()
                {
                    ...
                    ValorTotal = (decimal)((produto!.Preco) * Convert.ToDecimal(itemCarrinhoCompra.Quantidade))
                };
    ...
    public async Task<IActionResult> Put(int produtoId, string acao)
    {
        ...
        var usarioEmail = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Email)?.Value;

        var usuario = await dbContext.Usuarios.FirstOrDefaultAsync(u => u.Email == usarioEmail);
        if (usuario is null)
        {
            return NotFound("Usuario nao encontrado");
        }

        var itemCarrinhoCompra = await dbContext.ItensCarrinhoCompra.FirstOrDefaultAsync(s => s.ClienteId == usuario!.Id && s.ProdutoId == produtoId);
